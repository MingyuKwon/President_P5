...
| ✅ | `turn.js:checkRoundEnd` | 선(빈 테이블) 상황에서 패스 클릭 시 서버 TypeError 크래시 — `tableCards`가 빈 배열일 때 `getNum(undefined)` 호출로 crash | `checkRoundEnd` 시작 시 빈 tableCards 조기 반환 추가. 선 패스 불가 제한은 게임 규칙에 없으므로 적용하지 않음 |
