...
function renderSeats() {
  ...
  order.forEach((playerId, i) => {
    ...
    if (isMe) {
      ...
      div.style.cssText = 'position:absolute; left:calc(50% - 370px); bottom:calc(100% + 100px); top:auto; transform:translateX(-50%);';
...
function scaleGameBoard() {
  const scale = Math.min(window.innerWidth / 1200, window.innerHeight / 900);
  const board = document.getElementById('game-board');
  board.style.transform = `scale(${scale})`;
  board.style.left = `${(window.innerWidth - 1200 * scale) / 2}px`;
